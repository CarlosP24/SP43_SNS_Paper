function calc_mismatch_J(modL, modR, σ; Brng = subdiv(0.0, 0.25, 100), φs = subdiv(0, 2π, 51), τs = 0.1:0.1:1.0, path = "Output/tdisorder", d = 5, Lleft = 0, Lright = 0)
    ...
    # Get Greens
    g_right, g_left, g = greens_dict[gs](hSC_left, hSC_right, params_left, params_right, σ)

    bandwidth = maximum([model_left.Δ0, model_right.Δ0]) * 50
    J = josephson(g[attach_link[gs]], bandwidth; imshift = 1e-3, omegamap = ω -> (; ω), phases = φs, atol = 1e-3)
...
function calc_mismatch_LDOS(modL, modR, σ; Brng = subdiv(0.0, 0.25, 100), ωrng = subdiv(-.26, .26, 201) .+ 1e-4im, path = "Output/tdisorder", d = 5, Lleft = 0, Lright = 0)
