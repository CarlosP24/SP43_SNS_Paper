...
function plot_mismatch(;path = "Output/Rmismatch", L = 0)
    ...
    heatmap!(ax_left, Brng, ωrng, LDOS_left, colormap = :thermal, colorrange = (2e-3, 5e-2), lowclip = :black)
    hidexdecorations!(ax_left; ticks = false)

    ax_right = Axis(fig[2, 1]; ylabel = L"$\omega / \Delta_0$", yticks = ([-Δ0, 0, Δ0], [L"-1", "0", L"1"]))
    heatmap!(ax_right, Brng, ωrng, LDOS_right, colormap = :thermal, colorrange = (2e-3, 5e-2), lowclip = :black)
    ...
    for (i,τ) in enumerate(τs)
        ...
        lines!(ax_I, Brng, Ic ./ first(Ic); label = L"\tau = %$(τ)")
