## Option 1: combined
function TvI(pos; name::String, x::Real, Tspath = "data/Ts", bl = 0.5, br = 0.7, linewidth = 1.5, markersize = 5)
    ...
    scatter!(ax, Trng, Ics; color = :black, markersize)

    # Linear fit
    coef = bl * last(Ics) / last(Trng)
    lines!(ax, Trng, coef .* Trng; color = :blue, linestyle = :dash,  linewidth, label = L"$\propto T_N$")

    # Fit
    i = 1
    exp = 0.5
    coef = br * Ics[i]/ Trng[i]^exp
    br != 0 && lines!(ax, Trng, coef .* Trng.^exp; color = :red, linestyle = :dash, linewidth, label = L"$\propto \sqrt{T_N}$")
