...
function plot_Three(L; indir = "Output", geos = ["HCA", "MHC_20", "SCM"], channels = [8, 8, 28], cmin = 1e-4, cmaxs = [5e-2, 5e-2, 2e-1], sTcs = [0.15, 0.15, 0.08], Tcs = [0.3, 0.4, 0.7])
    ...
    for (col, (geo, cmax)) in enumerate(zip(geos, cmaxs))
        ...
        ax_Iτ = plot_Iτ(fig[3, col], indir_T)
        ...
        ax_Iτ.xticklabelsize = 10
        if col == 2
            text!(ax_Iτ, 1e-3, 2e-2; text = L"\sim \sqrt{T_N}", color = :red, rotation = 0.4, fontsize = 10, align = (:center, :center))
            text!(ax_Iτ, 5e-3, 8e-4; text = L"\sim T_N", color = :navyblue, rotation = π/4, fontsize = 10, align = (:center, :center))
        end 
    end
