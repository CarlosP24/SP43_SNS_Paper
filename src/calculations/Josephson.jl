...
function calc_Josephson(name::String, calc_params::Calc_Params)
    ...
    ipath1(B) = let bw = bw, Δ0 = wireL.Δ0, itip = itip, imshift = imshift
        return [-bw, -Δ0,  -Δ0/2 + itip(B)*1im, 0] .+ imshift*1im      # + imshift means retarded Greens. Advanced have a branch cut.
    end

    ipath2(B) = let bw = bw, Δ0 = wireL.Δ0, itip = itip, imshift = imshift
        return [-bw, -Δ0,  -Δ0/2 - itip(B)*1im, 0] .- imshift*1im     # - imshift means advanced Greens. Retarded have a branch cut.
    end
