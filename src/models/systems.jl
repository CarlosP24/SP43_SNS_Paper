...
systems_Rmismatch = Dict(
    ...
    j_params = i < 0.5 ? j_params_valve : j_params_valve_reduced,
...
systems_valve_majos_Andreev = Dict(
    "valve_majos_0" => System(systems_valve_majos["valve_majos"];
        calc_params = Calc_Params(systems_valve_majos["valve_majos"].calc_params; 
            ωrng = subdiv(-0.005, 0.0, 401) .+ 1e-7im,
            )
    ),
    "valve_majos_65" => System(systems_valve_majos["valve_majos"];
        calc_params = Calc_Params(systems_valve_majos["valve_majos"].calc_params; 
            ωrng = subdiv(-0.062, -0.061, 401) .+ 1e-9im,
            )
    ),
    "valve_majos_10" => System(systems_valve_majos["valve_majos"];
        calc_params = Calc_Params(systems_valve_majos["valve_majos"].calc_params; 
            ωrng = subdiv(-0.105, -0.995, 401) .+ 1e-7im,
            )
    ),
...
systems_ξLmismatch = Dict(
    ...
    wireL = wires["valve_65_ξ"], 
    wireR = wires["valve_65_ξ_100"],
    junction = Junction(; TN = i),
    j_params = j_params_valve,
    calc_params = calc_params_valve)
    for i in Ts_valve]
...
systems_ξLmismatch_trivial = Dict(
    ["ξLmismatch_trivial_$(i)" => System(; 
    wireL = wires["valve_trivial_65_ξ"], 
    wireR = wires["valve_trivial_65_ξ_100"],
    junction = Junction(; TN = i),
    j_params = j_params_valve,
    calc_params = calc_params_valve)
    for i in Ts_valve]
...
#systems_valve_majos = merge(systems_metal, systems_dep)
...
systems_valve_majos_full = merge(systems_valve_majos, systems_valve_majos_d)
