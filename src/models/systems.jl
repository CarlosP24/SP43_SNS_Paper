...
system_test_scm = Dict(
    ["scm_test_$(i)" => System(; wireL = (; wires["jos_scm"]..., Zs = 0), wireR = (;wires["jos_scm"]..., Zs = 0), junction = Junction(; TN = i), j_params = J_Params(; imshift = 1e-3, imshift0 = 1e-6, maxevals = 1e5), calc_params = Calc_Params(Calc_Params(); φrng = subdiv(0, 2π, 201), Φrng = [1])) for i in Ts]
)
