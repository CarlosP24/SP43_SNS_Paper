function loadres(name::String; path = "Results", length = "semi")
    ...
    resα = load("$(path)/$(name)_noSOC/$(length)_J.jld2")["resJ"]
    res = nResults(; params = resLDOS.params, junction = resLDOS.junction, LDOS_left = resLDOS.LDOS_left, LDOS_right = resLDOS.LDOS_right, Js_τs = resJ.Js_τs, Tτ = resT.Tτ, τT = resT.τT, Js_τs_σ = resσ.Js_τs, junction_σ = resσ.junction, Js_τs_α = resα.Js_τs)
    return res
end
